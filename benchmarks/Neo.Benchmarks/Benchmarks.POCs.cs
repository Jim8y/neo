...
namespace Neo
{
    public class BenchmarkPoCs
    {
        ...
        private static void Run(string poc)
        {
            ...
            using var snapshot = s_system.GetSnapshot();
            using var engine = ApplicationEngine.Create(TriggerType.Application, tx, snapshot, s_system.GenesisBlock, s_protocol, tx.SystemFee);
